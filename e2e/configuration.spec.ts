...
test.beforeAll(async () => {
  ...
  if (!address || typeof address === 'string')
    throw new Error('Configuration fixture server unavailable')
...
for (const empty of [false, true]) {
  test(`custom configuration renders and plays without sample assumptions (empty=${empty})`, async ({
    page,
  }) => {
    const errors: string[] = []
    page.on('pageerror', (error) => errors.push(error.message))
    await page.route('**/src/data/game.ts*', (route) =>
      route.fulfill({
        contentType: 'application/javascript',
        body: `export const gameLevels = ${JSON.stringify(empty ? [] : levels)}; export const gameLocations = [{ id: 'dunhuang', name: '自定义地点', title: '自定义探索标题', subtitle: '测试', coordinates: '0°', background_url: '/art/landscape.svg', levels: gameLevels }]; export const gameAuthors = []; export const siteConfig = ${JSON.stringify({ ...siteConfig, title: '自定义探索标题', backgroundUrl: '/art/landscape.svg' })};`,
      }),
    )
    ...
    if (empty) {
      ...
      await expect(page.locator('.chapter-preview img').last()).toHaveAttribute(
        'src',
        levels[4]!.panorama[0]!.url,
      )
      ...
      for (const [index, level] of levels.entries()) {
        ...
        await page
          .getByRole('button', {
            name: level.problems.length ? '开启谜题' : '完成本关',
            exact: true,
          })
          .click()
        ...
        await page
          .getByRole('button', {
            name: index < levels.length - 1 ? '完成本关 · 前往下一关' : '落款 · 查看探索回响',
          })
          .click()
      }
