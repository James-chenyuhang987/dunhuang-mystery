...
test('panorama hotspots reveal clues and a wrong answer advances without retry', async ({
  ...
  await expect(page.locator('.clue-toggle').nth(1)).toHaveAttribute('aria-disabled', 'true')
  await expect(page.locator('.clue-toggle').nth(2)).toContainText('未解锁')
  await page.locator('.panorama-hotspot').nth(1).click()
  await expect(page.locator('.clue-panel').nth(1).locator('.clue-body')).toBeVisible()
  await expect(page.locator('.clue-panel').first().locator('.clue-body')).toBeVisible()
