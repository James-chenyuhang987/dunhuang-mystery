...
for (const difficulty of [1, 2, 3]) {
  test(`selected middle chapter finishes after exactly tier ${difficulty} questions`, async ({
    page,
  }) => {
    ...
    const count =
      difficulty === 1
        ? 1
        : difficulty === 2
          ? Math.ceil(level.problems.length / 2)
    ...
    for (let index = 0; index < count; index++) {
      ...
      await page
        .getByRole('button', {
          name: index === count - 1 ? '查看本卷结果' : '下一道谜题',
          exact: true,
        })
        .click()
    }
...
test('difficulty can be selected before starting and is read-only during gameplay', async ({
  page,
}) => {
  await page.goto('/')
  await chooseDefaultLocation(page)
  const homeSlider = page.getByRole('slider', { name: '探索难度' })
  await homeSlider.fill('3')
  await expect(homeSlider).toHaveValue('3')
  await page.getByRole('button', { name: '选关', exact: true }).click()
  ...
  const settingsSlider = page.getByRole('slider', { name: '探索难度' })
  await expect(settingsSlider).toBeDisabled()
  await expect(settingsSlider).toHaveValue('2')
  await expect(page.locator('.game-settings .difficulty-lock')).toContainText('已锁定')
  await expect(page.locator('.game-settings [data-icon="lock"]')).toBeVisible()
  await page.getByRole('button', { name: '开启谜题', exact: true }).click()
  const dialog = page.getByRole('dialog')
  await expect(dialog.getByRole('slider', { name: '探索难度' })).toBeDisabled()
  await expect(dialog.getByRole('button', { name: '初探 1 题' })).toBeDisabled()
  await expect(dialog.getByRole('button', { name: '寻踪 2 题' })).toBeDisabled()
  await expect(dialog.getByRole('button', { name: '解谜 3 题' })).toBeDisabled()
})

test('fullscreen image supports zoom, pinch, reset and close without moving panorama', async ({
  page,
}) => {
  ...
  await expect(page.locator('.clue-toggle').nth(imageIndex)).toHaveAttribute(
    'aria-disabled',
    'true',
  )
