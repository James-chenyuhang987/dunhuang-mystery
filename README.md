...
- `gameLocations`：配置地点以及各自的关卡数组。默认含敦煌莫高窟和秦始皇帝陵博物院，各 3 个原创教育探秘关卡；开场结束后地球动画停留在地点选择界面。`gameLevels` 继续导出首个地点关卡以兼容旧调用。
- 每个关卡按类型定义填写全景 URL、线索及四选一问题。`true_answer` 从 **0** 开始。全景应使用 **2:1 等距柱状投影**，远程图片必须允许 CORS；建议单张不超过 8192×4096，并根据目标设备降低尺寸。
- `siteConfig`：配置网站标题、浏览器标题后缀、品牌、首页介绍、菜单及结算标题和默认背景。页面不再另存一份固定标题。
- 关卡可选字段 `subtitle`、`description`、`thumbnail_url`：用于二级菜单的副标题、简介、预览图；未填写时隐藏附加文字，预览图使用该关 `panorama_url`。章节编号按数组下标动态生成，不维护“一二三”映射，不拼接演示图片路径。重复关卡名称可用，身份依据数组下标；全部关卡正常渲染，无虚拟滚动。
...
首次进入首页 `/` 时，地点选择动画会停驻并等待选择；每个地点只加载自身的关卡。一级菜单仅提供“开始”和“选关”：开始从当前地点第一关按其 `levels` 数组顺序线性游玩，最后一关完成后结算；选关进入二级菜单 `/levels`，可选择任意一关及难度，该关完成后直接结算。所有关卡共享 `/game`，由 Pinia 保存当前地点、模式与关卡；结束与制作信息位于 `/ending`。继续上次进度与创作团队入口位于二级菜单。
...
拖动/单指旋转；滚轮/双指捏合改变 FOV；方向键和加减键也可操作。`hotspots` 使用配置中的屏幕百分比坐标显示可点击线索点，点选后展开相应 `clue_index`；`clue.problem_indexes` 可将线索关联至原始题目下标。线索默认折叠，支持图片、音频、视频、文本，面板事件不会传给全景。图片、视频提供全屏查看器，支持按钮、滚轮与双指缩放（100%–500%）、放大后拖动、重置及设备全屏。

每题只有一次作答机会：答错后记录结果但不直接公布正确答案，可打开关联线索后继续；也可跳过，答错与跳过分别统计并持久化。可选 `comparison` 任务允许上传图片并在浏览器本机生成 32×32 灰度亮度特征，以 0–100% 显示参考图相似度，图片不会上传服务器。该算法是轻量教学演示，不是 3D Gaussian Splatting 或语义 AI 鉴定。
