...
首页 `/` 的一级菜单只提供“开始”和“选关”：开始从第一关按 `gameLevels` 数组顺序线性游玩，最后一关完成后结算；选关进入二级菜单 `/levels`，可选择任意一关及难度，该关完成后直接结算。所有关卡共享 `/game`，由 Pinia 保存当前模式与关卡；结束与制作信息位于 `/ending`。继续上次进度与创作团队入口位于二级菜单。
