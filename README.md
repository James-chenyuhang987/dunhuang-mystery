...
- 每个关卡通过 `panorama: ImagePanorama[]` 定义全景画面；每项填写 `name`、普通纹理 `url`、可选 `ultraviolet_url`、`initial_view` 和 `click_points`。关卡可用 `timeline` 配置语义化节点，每个节点通过 `panorama_index` 和 `clue_indexes` 关联画面与线索；未配置时继续按全景名称生成兼容时间轴。纹理应使用 **2:1 等距柱状投影**，远程图片必须允许 CORS；建议单张不超过 8192×4096。问题 `true_answer` 从 **0** 开始。
...
拖动/单指旋转；滚轮/双指捏合改变 FOV；方向键和加减键也可操作。语义时间轴可让多个节点共用同一全景，点击节点会切换关联画面并高亮对应线索；未配置 `timeline` 的旧内容仍按 `panorama.name` 切换纹理。仅当当前画面定义有效的 `ultraviolet_url` 时显示紫外线按钮，切换画面会退出紫外模式。桌面端的时间轴、紫外线和发现进度位于底部工具栏，移动端通过“时间与观察”折叠面板打开。内置画境将 `ultraviolet_url` 设为当前 `url`，因此会保留真实全景并仅启用 ShaderPass 蓝紫色滤镜；旧故事仍可使用独立配套图。相同纹理会直接复用，不会在切换滤镜时重复下载。开启紫外线后，紫外专属点击点会显示为可交互的调查标记。纹理加载失败会退出紫外模式，异步过期结果及后处理/WebGL 资源均会释放。

`ClickPoint.vec` 是以球心为原点、半径约 10 的球面笛卡尔坐标；点击射线与球面的交点减去球心后，与同模式点击点计算欧氏距离，距离小于等于 `accept_click_range` 即命中。每次非拖动点击都会在浏览器控制台输出实际 `Vector3` 和可直接粘贴到配置的 `new Vector3(x, y, z)`。`in_uv: false` 仅普通模式可发现，`true` 仅紫外模式可发现；重叠范围取最近点。发现按关卡、画面和配置下标去重并持久化。底部统计优先以本关线索总数和已解锁线索计算，旧内容中的隐藏点击发现继续作为兼容进度来源。
...
人物对话也是一种线索类型。将点击点的 `dialogue_id` 与线索中的 `dialogue` 决策树关联后，玩家点击全景中的人物即可解锁对话；对话支持人物、头像、逐步推进和分支选项。结算页会根据地点或故事封面生成带图明信片，可下载或调用系统分享。

“故事工坊”是独立于敦煌和云冈的本地创作空间，可从入口选择或访问 `#/studio`。用户可以编辑故事信息、创建和排序关卡、填写全景地址、添加点击点、线索和人物对话，并将故事导出为版本化 JSON；故事草稿使用独立的 localStorage 存储，不会覆盖正式游戏存档。
...
```sh
npm run prettier       # 全局格式化 Prettier 支持的源码、配置和文档
npm run prettier:check # 仅检查格式，不修改文件
```

格式规则见 `.prettierrc.json`：单引号、无分号、100 字符换行宽度。`.prettierignore` 排除依赖、构建产物、测试报告、演示生成临时文件、公共媒体素材及交付文件；不支持的文件类型自动跳过。
