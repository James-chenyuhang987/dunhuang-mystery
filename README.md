...
拖动/单指旋转；滚轮/双指捏合改变 FOV；方向键和加减键也可操作。语义时间轴可让多个节点共用同一全景，点击节点会切换关联画面并高亮对应线索；未配置 `timeline` 的旧内容仍按 `panorama.name` 切换纹理。仅当当前画面定义有效的 `ultraviolet_url` 时显示紫外线按钮，切换画面会退出紫外模式。桌面端的时间轴、紫外线和发现进度位于底部工具栏，移动端通过“时间与观察”折叠面板打开。`ultraviolet_url` 可提供灰度或彩色原图；渲染器先按 sRGB 解码，再由 ShaderPass 映射紫外配色，并分别增强低色差白色标记与暖金色题记的辉光。纹理成功后才替换旧纹理；加载失败会退出紫外模式，异步过期结果及后处理/WebGL 资源均会释放。
