...
export function useSceneManager(options: SceneManagerOptions): SceneManagerApi {
  ...
  const loadTexture = () => {
    const id = ++loadId,
      source = assetUrl(options.isUltraviolet() ? options.getUltravioletUrl() : options.getUrl()),
