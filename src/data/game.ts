...
const dunhuangLevels: level[] = [
  {
        ...
        click_points: [
          {
            ...
            name: '蓝签残片',
            description: '球面中央的蓝签残片与交接簿记载相互呼应。',
            image: '/art/clue.svg',
            in_uv: false,
          },
          {
        ...
        click_points: [
          {
            ...
            accept_click_range: 1.1,
            name: '匣位刻痕',
            description: '旧档复原画面里的细小刻痕，为摹本流转提供旁证。',
            in_uv: false,
          },
        ],
      },
    ...
    hotspots: [
      ...
      { clue_index: 1, vec: new Vector3(10, -0.5, 0) },
      { clue_index: 2, vec: new Vector3(10, 1.2, 2.1) },
    ],
    clues: [
      {
        type: 'image',
        ...
        problem_indexes: [0],
        data: '/art/clue.svg',
        hint: '虚构交接簿记载：甲匣原存蓝签摹本，乙匣原存红签摹本。交接时蓝签摹本暂移至乙匣，归还记录尚未填写。图像为教学示意。',
      },
      {
        type: 'text',
        name: '保管员便笺',
        problem_indexes: [1],
        data: '纸签颜色只能提示分类，不能说明移动经过。请核对交接记录，并向当值保管员了解情况。',
      },
      {
        type: 'text',
        name: '现场勘查记录',
        problem_indexes: [2],
        data: '本案为虚构教学故事。现场未见破锁或撕裂，也没有指认行为人的直接证据。记录缺漏与盗窃是不同的判断，不能互相替代。',
      },
    ],
    problems: [
      {
        title: '按交接簿，蓝签摹本最后被记载移到了哪里？',
        select: ['甲匣', '乙匣', '洞外沙地', '无法判断任何位置'],
        true_answer: 1,
        reason:
          '交接簿明确写着“蓝签摹本暂移至乙匣”，这是案内最后一条位置记录，并不证明它现在仍在那里。',
      },
      {
        title: '发现蓝色纸签后，最可靠的下一步是什么？',
        select: ['立即指认盗窃者', '按颜色猜测年代', '核对交接簿并询问保管员', '将纸签带走收藏'],
        true_answer: 2,
        reason:
          '颜色仅提示分类；交叉核对记录与当事人证词，比凭单一线索下结论可靠，也避免触碰与带走物品。',
      },
      {
        title: '哪一项是这份案卷尚不能支持的结论？',
        select: ['甲匣原存蓝签摹本', '蓝签摹本曾暂移', '归还记录未填写', '某位保管员偷走了摹本'],
        true_answer: 3,
        reason:
          '没有破锁或撕裂，且没有关于行为人的直接证据。记录缺漏不等同于盗窃，更不能据此指认个人。',
      },
    ],
  },
  {
    ...
    hotspots: [
      { clue_index: 0, vec: new Vector3(10, 0.7, -2.1) },
      { clue_index: 1, vec: new Vector3(10, -0.9, -0.7) },
      { clue_index: 2, vec: new Vector3(10, 1.4, 0.9) },
      { clue_index: 3, vec: new Vector3(10, -0.35, 2.1) },
    ],
    clues: [
      {
        type: 'text',
        name: '展签顺序记录',
        problem_indexes: [0],
        data: '教学展签依次写着“救助”“承诺”“背弃”。柜门要求取第二张展签上的两个字。',
      },
      {
        type: 'text',
        name: '策展人的故事卡',
        problem_indexes: [1],
        data: '受助者许诺不泄露救助者的所在，后来却为了奖赏泄露了位置。比较许诺的内容与后来的行为。',
      },
      {
        type: 'text',
        name: '褪色观察记录',
        problem_indexes: [2],
        data: '记录褪色区域的位置、范围与变化，交由专业人员判断；不自行擦拭、描补或揭取表层。',
      },
      {
        type: 'image',
        name: '教学展签说明',
        problem_indexes: [3],
        data: '/art/clue.svg',
        hint: '展签、柜门和密码均为本游戏编写。图示帮助理解教学谜题，不能据此推断真实文物也包含相同密码。',
      },
    ],
    problems: [
      {
        title: '依照修复记录，柜门的两个字应当是什么？',
        select: ['救助', '承诺', '背弃', '奖赏'],
        true_answer: 1,
        reason: '提示要求取第二张展签，记录中第二项是“承诺”。这只是案内编写的文字谜题。',
      },
      {
        title: '故事卡中，受助者的行为为何与承诺冲突？',
        select: ['他为奖赏泄露救助者所在', '他拒绝领取奖赏', '他保护了救助者', '他忘记展签颜色'],
        true_answer: 0,
        reason: '承诺是不泄露所在；为了奖赏而泄露，正与这一承诺相违背。',
      },
      {
        title: '对于教学记录提到的褪色，哪种做法合适？',
        select: [
          '凭想象涂满颜色',
          '擦拭确认颜料',
          '记录现象并交由专业人员判断',
        ...
        true_answer: 2,
        reason: '保护文化遗产应避免擅自接触、擦拭或修补。记录并求助专业人员是更审慎的做法。',
      },
      {
        title: '这关的展签密码能证明什么？',
        select: [
        ...
        true_answer: 3,
        reason:
          '教学谜题不是历史证据；展签顺序与柜门密码都由游戏编写，不能推导真实文物具有同样机制。',
      },
    ],
  },
  {
        ...
        click_points: [
          {
            ...
            accept_click_range: 0.9,
            name: '重合纤维',
            description: '紫外观察显示两段纸纤维的走向能够衔接。',
            in_uv: true,
          },
        ],
      },
    ...
    clues: [
      {
        ...
        name: '虚构整理室日志',
        problem_indexes: [0],
        data: '教学整理室有三份数字摹本记录：A为入库，B为临时借阅，C为归还。时间先后是A、B、C；C备注“已归还原位，编号待核”。',
      },
      {
        type: 'text',
        name: '核验清单',
        problem_indexes: [1],
        data: '先核对编号，再核对交接时间，最后联系记录人确认。只有三者吻合才能确认流转；仍有疑点时保留“待核验”，不能凭猜测补齐证据。',
      },
      {
        type: 'image',
        name: '数字摹本传播说明',
        problem_indexes: [2],
        data: '/art/clue.svg',
        hint: '传播数字摹本需保留出处、使用授权和教学材料性质。不能把教学摹本当作新发现的真迹，也不能把游戏剧情当成历史结论。',
      },
    ],
    problems: [
      {
        title: '哪份记录在案内时间线上最后出现？',
        select: ['A：入库', 'B：临时借阅', 'C：归还', '三份没有时间顺序'],
        true_answer: 2,
        reason: '日志明确给出A、B、C的先后顺序，因此C是最新记录；其“编号待核”仍需进一步检查。',
      },
      {
        title: '看到“已归还原位，编号待核”，应怎样处理？',
        select: [
          '立即宣布文物被盗',
          '先核对编号、交接时间并联系记录人',
          '删除之前的记录',
        ...
        true_answer: 1,
        reason: '“待核”说明证据尚不完整。应按核验清单交叉确认，而不是把不确定信息变成既定事实。',
      },
      {
        title: '传播这些数字摹本时，哪种说明最准确？',
        select: [
          '它们都是本游戏新发现的真迹',
          '不用标明来源即可商用',
          '游戏结案等于历史定论',
        ...
        true_answer: 3,
        reason:
          '来源、授权和材料性质应清晰区分。本案是虚构教育推理，不能以教学图像或故事宣称历史发现。',
      },
    ],
  },
]
