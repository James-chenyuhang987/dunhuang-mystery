...
const props = withDefaults(defineProps<{ levelIndex: number; editable?: boolean }>(), {
  editable: true,
})
const game = useGameStore()
const tiers: { label: string; value: Difficulty }[] = [
  { label: '初探', value: 1 },
  { label: '寻踪', value: 2 },
  { label: '解谜', value: 3 },
]
...
function count(value: Difficulty): number {
  return value === 1
    ? Math.min(1, total.value)
    : value === 2
      ? Math.ceil(total.value / 2)
      : total.value
}
  ...
  <div class="difficulty-control" :class="{ readonly: !editable }">
    <div class="field-heading">
      <span class="difficulty-label"
        >探索难度<span v-if="!editable" class="difficulty-lock"
          ><AppIcon name="lock" />已锁定</span
        ></span
      >
      <output class="tiny" aria-live="polite"
        >本关需答 {{ count(game.difficulty) }} / {{ total }} 题</output
      >
    </div>
    <input
      aria-label="探索难度"
