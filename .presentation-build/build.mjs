...
}
{
 ...
 note(s,'可以把它想成站在一个内壁贴满照片的球里。等距柱状图把水平角和垂直角对应到图片坐标，完整覆盖360度乘180度，所以通常是2比1。Three.js生成球体，再反转X轴使内侧可见。相机留在球心，从一个方向看过去，就得到当前屏幕画面。单纯把普通照片拉成2比1不会变成正确的全景。项目中的半径10是场景单位，不代表真实洞窟尺寸。', ['src/components/PanoramaViewer.vue：initialize()，SphereGeometry(10,64,32)、geometry.scale(-1,1,1)。','https://threejs.org/docs/pages/SphereGeometry.html','图：public/yungang/yungang_cave3_pano.jpg。球体插图为本次生成的概念示意，不是真实洞窟模型。']);
}
{
 ...
 note(s,'拖动时，代码修改水平角和垂直角，再让相机看向新的方向。缩放时修改FOV，也就是相机的垂直视野角：数值小，屏幕装下的范围更窄，物体就显得更大。这类似在原地用长焦观察，并没有走到物体前面。代码把FOV限制在30到100度。两个截图来自同一关卡、同一朝向。', ['src/components/PanoramaViewer.vue：move()、zoom()、render()。','https://threejs.org/docs/pages/PerspectiveCamera.html','图：本次本地运行截图，70度和40度。']);
}
{
...
}
{
 ...
 note(s,'两种技术的差别在于怎样表示场景。全景把一个观察点看到的颜色存进图片，旋转相机就能环顾。3D高斯使用大量位置、形状、颜色和不透明度可调整的高斯体，从多视角照片拟合场景，再把它们投影到新视角并混合成图像。因此它能支持观察位置变化，但效果取决于拍摄覆盖、相机位姿和优化质量。原稿记录我们受采集条件限制，选择全景方案。这是本项目的取舍，不是说全景在所有场景下都比3DGS更好。当前代码没有高斯模型或渲染器。', ['团队选型：docs/展示PPT.pptx，第7页。','实现核验：src/components/PanoramaViewer.vue、package.json。','Kerbl等，2023，3D Gaussian Splatting for Real-Time Radiance Field Rendering：https://repo-sam.inria.fr/fungraph/3d-gaussian-splatting/']);
}
{
 ...
 note(s,'这里有两个步骤。首先把普通纹理url换成专用的ultraviolet_url，然后启用紫外线着色器。专用素材已经画好了白色标记。着色器让背景偏紫蓝、让白色标记显得更亮。它不会从一张普通照片自动发现真实隐藏文字，也没有在测量真实紫外光。当前截图使用原创教学SVG，代表游戏里的视觉模拟。只有当前时相提供了专用素材，界面才允许开启这个模式。', ['src/components/PanoramaViewer.vue：loadTexture()。','src/views/GameView.vue','src/data/game.ts：九色秘语的url与ultraviolet_url。','public/art/cave-02.svg、public/art/dunhuang-uv.svg。','图：同一朝向的本地项目截图，素材和效果均来自现有实现。']);
}
{
 ...
 note(s,'Shader可以理解成一段让GPU对每个像素执行的计算规则。代码先把sRGB纹理解码，在线性颜色空间里用加权RGB算亮度，再用最大分量减最小分量估计色差。白色既亮又没有明显色偏，所以用高亮度、低色差作为荧光候选。smoothstep让阈值平滑过渡。背景按明暗映射到紫蓝色，候选标记映射到亮青色。再取上下左右四个邻点的白色强度做平均，叠加一圈小范围辉光。流程是RenderPass渲染场景、ShaderPass改颜色、OutputPass处理最终输出。它是一种自定义后处理滤镜，不能判断文物的年代、材质或真实隐迹。', ['src/components/PanoramaViewer.vue：ultravioletShader、whiteness()、ultravioletColor()、initialize()。','https://threejs.org/docs/pages/ShaderPass.html','https://threejs.org/docs/pages/EffectComposer.html','补充：uvLuminance()权重为0.2126、0.7152、0.0722。邻域步长为1.5/resolution。']);
}
{
 ...
 note(s,'除了画面效果，我们也处理真实使用中的问题。异步加载用编号区分新旧请求，避免旧请求覆盖新画面；纹理成功后才替换，并释放旧资源。普通模式直接渲染，紫外模式才运行后处理。存档在关键动作和页面离开时保存，另外每15秒备份。强制结束浏览器时，最后一小段计时仍可能丢失。项目有Vitest和Playwright测试，不能把存在测试等同于完全没有错误。虽然仓库有切片工具，当前渲染器还没有实现分级瓦片加载。', ['src/components/PanoramaViewer.vue：renderPixelRatio()、render()、loadTexture()、dispose()。','src/App.vue','src/stores/game.ts','tools/slice-panoramas.py','vitest.config.ts、playwright.config.ts。']);
}
{
...
const final=path.join(root,'docs','敦煌探迹_项目与核心原理.pptx');
