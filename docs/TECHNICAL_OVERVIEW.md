...
3. `GameView` 编排 `PanoramaViewer`、线索抽屉、题目弹窗和语义时间轴；时间轴节点可关联同一全景中的不同线索。
...
- 低色差、高亮度区域作为白色“荧光”候选；暖色且红绿分量明显高于蓝色的区域作为镀金候选。两类候选都会采样上下左右邻域，分别形成蓝色与暖金色辉光。
- UV 只在当前时相存在 `ultraviolet_url` 时开放；切换时相会退出 UV，避免显示错误状态。
- 每张全景可提供 `initial_view`，在进入画面时恢复适合该洞窟的经纬视角与 FOV，确保关键热点不会默认落在相机背面。
...
- **可维护数据**：关卡、全景、语义时间轴、点击点、热点、线索和题目均配置驱动，新增地点主要修改 `src/data/game.ts`。
